...
def state_retail(state):
    api_key = EIA_API_KEY
    json_file = os.path.join(BASE_DIR, 'static/js/data/retail/{}.json'.format(STATES[state][1]))
    ...
    state_json_file = os.path.join(BASE_DIR, 'static/js/data/states/{}.json'.format(STATES[state][1]))
    with open(state_json_file, 'r') as f:
        state_json = json.load(f)
        state_json['policy_and_price'] = state_json.get('price_and_policy', {})
        state_json['policy_and_price']['policy_retail'] = int(round(data[-1]['data'] * 10, 0))
...
def state_gridmix(state):
    ...
    eia = {
        ...
        'series_list': [
            ...
            {'sector': 'Distillate Fuel Oil', 'code': 'DFTCB', 'intensity': 670},
            ...
            {'sector': 'Natural Gas as Lease and Plant Fuel', 'code': 'NGLPB', 'intensity': 400},
            ...
            {'sector': 'Supplemental Gaseous Fuels', 'code': 'SFTCB', 'intensity': 9999},
