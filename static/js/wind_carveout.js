var Trajectory = function() {
  var data={trajectory:null, wind:null, solar:null, current:null, min:null, max:null},
    ...
    domain_y = [0, 50],
    adjust_data = {step:.5, start:0, stop:0, index:0, max:0},
    adjust_dot,
  ...
  function drag_move(d, i, ii) {
    var adjusted = y.invert(d3.event.y),
      ...
      rounded = (delta > 0) ? (delta > adjust_data.max) ? adjust_data.max : delta : 0
    ;
  ...
  function drag_start(d, i, ii, foo) {
    ...
    adjust_data.max = data.max.filter(function(_d) { return (_d.date == d.date)}).data;
    adjust_data.index = i;
  ...
  function redraw() {
    trajectory
      .attr('d', trajectory_area(data.wind)) //changes
    ;
  ...
  function trajectory_graph(_s) {
    ...
    data.wind = _s.properties.carveouts.filter(function(d) { return (d.type == 'wind') ? d : null; })[0].data;
    data.current = data.wind;
    data.max = data.trajectory;
    trajectory = graph.append('path')
      .attr('d', trajectory_area(data.wind))
      .attr('class', 'chart-line')
      .style('fill', d3.rgb(86, 180, 233))
    ;
    ...
    grid_layer.append('path')
      .attr('d', trajectory_area(data.trajectory))
      .attr('class', 'chart-line')
//      .style('fill', d3.rgb(0, 158, 115).brighter())
      .style('fill', '#dddddd')
    ;
    ...
    dflt.append('path')
      ...
      .attr('d', trajectory_line(data.wind)) //changes
    ;
